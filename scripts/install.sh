...
# 等价于 README 的「手动安装」步骤,全部幂等,可安全重复执行。
...
BETTER_SOURCE="https://raw.githubusercontent.com/omdsh-dev/DSH-better-sidebar/main/scripts/install.sh"
...
(
  cd "$PROFILE_DIR"
  if [ -d "node_modules/$BETTER" ]; then
    info "  已安装 $BETTER,跳过"
  else
    warn "未检测到 $BETTER,先执行其官方一键安装…"
    if [ "$DRY_RUN" = true ]; then
      printf '\033[0;90m  · curl -fsSL %s | bash\033[0m\n' "$BETTER_SOURCE"
    else
      info "▸ curl -fsSL $BETTER_SOURCE | bash"
      curl -fsSL "$BETTER_SOURCE" | bash
      if [ "$?" -ne 0 ]; then
        err "上游 $BETTER 安装失败。请先手动完成它（curl -fsSL $BETTER_SOURCE | bash）再重跑本脚本。"
      fi
      if [ "$PROFILE" != "web" ]; then
        warn "注意：上游官方脚本默认安装到 web profile；你在用 $PROFILE,"
        warn "若需要 $BETTER 也装到 $PROFILE,请补执行：dsh plugin --profile '$PROFILE' add '$BETTER'"
      fi
    fi
  fi
)
